    ...
    <script type="text/javascript" charset="utf-8">
      ...
      desc = desc.replace(/#([^#]+)/g, '<span class="desc-b">$1</span>'); //initially yellow
        ...
        function updateItemDetails(val, data) {
            ...
            if (val.j !== null || data.incSTR !== null) {val.j = updateStat(val.j, data.incSTR, e); }
            if (val.k !== null || data.incDEX !== null) {val.k = updateStat(val.k, data.incDEX, e); }
            if (val.l !== null || data.incINT !== null) {val.l = updateStat(val.l, data.incINT, e); }
            if (val.m !== null || data.incLUK !== null) {val.m = updateStat(val.m, data.incLUK, e); }
            if (val.n !== null || data.incMHP !== null) {val.n = updateStat(val.n, data.incMHP, e); }
            if (val.o !== null || data.incMMP !== null) {val.o = updateStat(val.o, data.incMMP, e); }
            if (val.p !== null || data.incPAD !== null) {val.p = updateStat(val.p, data.incPAD, e); }
            if (val.q !== null || data.incMAD !== null) {val.q = updateStat(val.q, data.incMAD, e); }
            if (val.r !== null || data.incPDD !== null) {val.r = updateStat(val.r, data.incPDD, e); }
            if (val.s !== null || data.incMDD !== null) {val.s = updateStat(val.s, data.incMDD, e); }
            if (val.t !== null || data.incACC !== null) {val.t = updateStat(val.t, data.incACC, e); }
            if (val.u !== null || data.incEVA !== null) {val.u = updateStat(val.u, data.incEVA, e); }
            if (val.v !== null || data.incCraft !== null) {val.v = updateStat(val.v, data.incCraft, e); }
            if (val.w !== null || data.incSpeed !== null) {val.w = updateStat(val.w, data.incSpeed, e); }
            if (val.x !== null || data.incJump !== null) {val.x = updateStat(val.x, data.incJump, e); }
            if (val.C !== null || data.bdR !== null) {val.C = updateStat(val.C, data.bdR, e); }
            if (val.D !== null || data.imdR !== null) {val.D = updateStat(val.D, data.imdR, e); }
        }
            ...
            if (stat != null) {
                if ($.isArray(stat)) {
                    if (stat[0] > stat[1] || (stat[1] == null && stat[0] > 0)) { // Higher
                        result = rowStart + '<span class="text-primary">' +
                                prefix + stat[2] + suffix + '</span>' + rowEnd;
                    } else if (stat[1] > stat[0] || (stat[0] == null && stat[1] > 0)) { // Lower
                        result = rowStart + '<span class="text-danger">' +
                                prefix + stat[2] + suffix + '</span>' + rowEnd;
                    } else {
        ...
        function getItemDetails(iRow, val) {
            ...
            if (val.y != null) {itemDetails += rowStart + 'Growth Level: ' + 1 + rowEnd;}
            ...
            if (val.h != null) {itemDetails += rowStart + 'NUMBER OF UPGRADES AVAILABLE: ' + val.h + rowEnd;}
            if (val.H != null && val.H > 0) {itemDetails += rowStart + val.H + ' Enhancement Applied' + rowEnd;}
            if (val.A != null) {itemDetails += rowStart + 'NUMBER OF HAMMER APPLIED: ' + val.A + rowEnd;}
            ...
            if (val.V) {
                itemDetails += divider;
                itemDetails += rowStart + '<span class="text-success">' +
                    (val.V.length > 0 ? val.V : 'You can mount a Nebulite on this item.') +
                    '</span>' + rowEnd;
            }
        ...
        $(document).ready(function() {
            ...
            // Add stat filters
            var statFilter = new CodePeg.StatFilter($('#stat-filter'), {
                ...
                content: function() {
                    ...
                    if (iRow in iRowMap) { // If this row exists (why wouldn't it?)
                        ...
                        if (val.hasDetails) { // Item already has details
                            ...
                            $.getJSON("/api/items?id=" + val.U, function(data) {
                                data = data.item;
...
<?php /*
            // Insert a details column for expanding / collapsing item stats
...
?>
            ...
            var oTable = $('#search_results_table').dataTable( {
                ...
                "aoColumns": [
                    { "bSearchable": true},  // Item
                    { "bSearchable": false}, // Qty
                    { "bSearchable": false}, // Bundle
                    { "sType": "numeric-formatted", "bSearchable": false}, // Price
                    { "bSearchable": false}, // Ch
                    { "bSearchable": false}, // Rm
                    { "bSearchable": false, "bVisible": false}, // Shop
                    { "bSearchable": true},  // Seller
                    { "bSearchable": false, "bVisible": false}, // Median
            ...
            $("#select-category").change(function() {
                ...
                if (category != '') {
                    $subCategory.empty();
                    $subCategory.append('<option value="">All ' +
                            category + '</option>');
                    var subs = [];
                    ...
                    if (i > -1) {
                        do {
                            $subCategory.append('<option value="' + subs[i] +
                                '">' + subs[i] + '</option>');
                        } while (--i >= 0);
                    }
                }
            ...
            $("#select-subcategory").change(function() {
                ...
                if (subCategory != '') {
                    ...
                    $detailCategory.append('<option value="">All ' +
                            subCategory + '</option>');
                    var details = [];
                    ...
                    if (i > -1) {
                        do {
                            $detailCategory.append('<option value="' + details[i] +
                                    '">' + details[i] + '</option>');
                        } while (--i >= 0);
                    }
                }
                ...
                $.each(data, function(key, val) {
                    ...
                    if (fm_items != null) {
                        ...
                        $.each(fm_items, function(key, val) {
                            ...
                            var name = '<div class="item-popover" data-toggle="popover" ' +
                                    'data-title="' + itemTitle +
                                    '" data-class="' + rareClass + '" data-row="' + iRow +
                            ...
                            if (shopName.length > 12) {
                                ...
                                shopName = '<div class="item-tooltip" data-toggle="tooltip" data-title="' +
                                        htmlEntities(shopName) + '">' + shopName + '</div>';
                            }
                        ...
                        var time = new Date(<?php echo $curTime?> - seconds_ago * 1000);
                        ...
                        $(".attach-header").after(
                            '<span style="font-size:16px;color:#AAA;">Updated <b><span id="minute">' + minuteOffset + '</span> minute' +
                                (minuteOffset == 1 ? '' : 's') + '</b> ago' +
                                extraText +
                            '</span>'
                        );
                    }
